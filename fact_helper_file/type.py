...
def get_file_type_from_path(file_path: str | Path) -> dict:
    """
    ...
    :param file_path: The path to the file to whose type info should be retrieved.
    """
...
def _get_type_from_magic_object(path_or_binary, magic_object, function_name, mime=True):
    try:
        ...
        logging.error(f"Could not determine file type: {type(exception)} {exception!s}")
